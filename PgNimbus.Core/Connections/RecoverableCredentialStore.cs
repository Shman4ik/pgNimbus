...
public sealed class RecoverableCredentialStore(ICredentialStore persistent, string? legacyDirectory = null) : ICredentialStore
{
    ...
    {
        get { lock (_gate) return _warnings.Count == 0 ? null : string.Join("\n", _warnings.Values.Distinct()); }
    }
    ...
    private void Warn(Guid id) => _warnings[id] = "Password storage is unavailable or migration could not finish. You can connect using the password in this dialog, but changes may last only for this session. Check your OS credential store (Keychain on macOS; Secret Service and libsecret-1 on Linux), then save again. Existing legacy credential files are kept until migration is verified.";
    private void WarnLegacy(Guid id) => _warnings[id] = "An old unencrypted credential file remains. Save this connection again to verify the OS-stored password and remove the legacy file.";
