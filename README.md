...
- **Row details and filter chips (optional).** Turn them on in Preferences. <kbd>Ctrl</kbd>+<kbd>I</kbd> opens the selected row as a form, with the grid's type-aware editors; its edits are staged and go through the same review and conflict-checked commit as Safe mode. While browsing, <kbd>Ctrl</kbd>+<kbd>F</kbd> adds a typed condition or NULL test as a chip, shows the SQL it adds, and runs it on the server.
...
- [x] **T4 · Row detail editor and visual filters (M).** Both are optional, behind one preference that is off by default. <kbd>Ctrl</kbd>+<kbd>I</kbd> opens the selected row as a keyboard-driven form over the window, reusing the Add-row dialog's type-aware editors; its edits always stage and go through the existing review and conflict-checked commit. Table browsing gains filter chips with type-dependent comparisons and NULL tests; each shows its SQL before it runs and is applied on the server as part of the page query. Filters exist only in browse mode, so a hand-written query is never rewritten. See [Filtering rows](docs/guide/results.md#filtering-rows) and [Row details](docs/guide/results.md#row-details).
