...
- **No-SQL table browsing.** Paged browsing with click-to-sort headers, all pushed down to Postgres (`ORDER BY`/`LIMIT`/`OFFSET`), so a huge table stays as cheap as one page. A filter bar (<kbd>Ctrl</kbd>+<kbd>F</kbd> in the grid) adds typed conditions and NULL tests, shows the `WHERE` it will run, and runs it on the server. The composed SQL sits in the editor; a query you write yourself is never rewritten.
- **Row details.** <kbd>Ctrl</kbd>+<kbd>I</kbd> shows the selected row as a name/value form beside the grid, with the grid's type-aware editors. Its edits are staged and go through the same review and conflict-checked commit as Safe mode.
...
- [x] **T4 · Row detail editor and visual filters (M).** <kbd>Ctrl</kbd>+<kbd>I</kbd> opens a keyboard-driven name/value sidebar that reuses the Add-row dialog's type-aware editors; its edits always stage and go through the existing review and conflict-checked commit. Table browsing gains a filter bar with type-dependent comparisons, NULL tests and a live `WHERE` preview, applied on the server as part of the page query. Filters exist only in browse mode, so a hand-written query is never rewritten. See [Filtering rows](docs/guide/results.md#filtering-rows) and [Row details](docs/guide/results.md#row-details).
