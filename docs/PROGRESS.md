...
| FK-aware JOIN magic (the flagship) | Two pieces, both driven by a new `SchemaService.GetForeignKeysAsync` (one `pg_constraint contype='f'` query across all schemas, `unnest(conkey, confkey) WITH ORDINALITY` zipping composite-key columns positionally into a `ForeignKeyInfo`). **(1) Ranked JOIN targets**: `SqlCompletionContext` now classifies `JOIN` as its own `SqlClause.JoinTableRef` (split out from the FROM/INTO/UPDATE `TableRef` bucket), and `SqlCompletionProvider.GetJoinTableRefCompletions` floats every table FK-adjacent to one already in the statement (either side of the relationship) above the flat catalog dump, tagged "· FK" in the description. **(2) The join condition itself**: a new `IsAfterOnKeyword` heuristic (distinct from the other `Predicate`-clause keywords WHERE/HAVING/BY/USING) fires right after `ON `, and offers `child.fk_col = parent.pk_col` (AND-joined for a composite key) as the single top, preselected item — accepting it with Enter completes the whole condition in one keystroke, exactly the `oi.order_id = o.id` example from the backlog text. The graph-matching itself (`FindJoinCandidates`/`BuildJoinCondition`, given parsed table+alias tuples and the FK edge list) is pure data-in/data-out logic, so it moved to a new **Core** class `PgNimbus.Core.Schema.ForeignKeyMatcher` rather than staying in the App-only `SqlCompletionProvider` — the SQL-*text* parsing that produces its inputs (`SqlCompletionContext`) stays in App since it exists only to serve the editor popup, but the graph walk needed no Avalonia/App types and is exactly the kind of logic CLAUDE.md's completion-testing guidance wants unit-tested. Also: fixed a latent gap the flagship's "one-keystroke" claim exposed — the auto-open-on-space paths (FROM/JOIN/now-ON) never preselected anything either, same root cause as the first-keystroke bug but never triggered before since nobody complained about auto-open specifically; `ShowCompletion`'s `SelectItem` replay is now unconditional (empty-string filter still ranks by priority, so the FK item or the first table wins the tie). **Tests**: 11 new TUnit cases in `ForeignKeyMatcherTests` (alias vs. bare-name conditions, either-side-child resolution, composite-key AND-joins, searching past a table with no direct FK, both directions of `FindJoinCandidates`, dedup, exclusion of already-referenced tables) — full suite now 43/43. **Live-verified** (Debug build, `pgnimbus-test` Docker Postgres seeded with `sales.orders → public.customers` and `sales.order_items → sales.orders` FKs) in both themes: `orders o JOIN ` floated `order_items`/`customers` (both "· FK") above the plain catalog; accepting `order_items oi ON ` auto-opened with `oi.order_id = o.id` preselected, Enter inserted it, and the resulting query (`… JOIN sales.order_items oi ON oi.order_id = o.id`) ran and returned 5,000 rows; regression-checked FROM auto-open, `o.` member access, `coale→coalesce()`, and string-literal suppression all still intact. | `(this commit)` |

| Fuzzy matching + smarter ranking | The stock AvaloniaEdit filter (prefix/substring `GetMatchQuality`) can't be swapped — `CompletionList.SelectItem` isn't virtual and `CompletionWindow.CompletionList` is read-only — so it's disabled (`IsFiltering = false`, leaving only a harmless selection-only pass) and `MainWindow.ApplyFuzzyFilter` owns filtering: on open and on every `Caret.PositionChanged` while the popup lives, it re-ranks the provider's candidate snapshot against the typed segment and pushes the result into the list (mutating `CompletionData` *and* reassigning `ListBox.ItemsSource`, since the data list is bound once at template time — the same trick the stock filter path uses). The ranking itself is pure logic in **Core**: `FuzzyMatcher` moved from the App (command palette keeps using it) to `PgNimbus.Core.Text`, joined by `CompletionRanker` (subsequence match; ties broken exact-prefix → context priority → shorter name → recency → stable) and `CompletionRecency` (bounded most-recent-first accept log, fed from the popup's `InsertionRequested`). Context priority deliberately outranks shorter-name so the statement's own columns don't lose to short catalog names; an empty query (auto-open) keeps provider order and preselects by priority, exactly the pre-existing behavior. New behavior: a query matching nothing now closes the popup (stock kept it open, unfiltered). **Tests**: 23 new TUnit cases (`CompletionRankerTests` 12 — both README examples asserted: `dr` finds `daily_revenue`, `ord` preselects `orders` over `order_items` —, `CompletionRecencyTests` 5, `FuzzyMatcherTests` 6); full suite 69/69. **Live-verified** (Debug build, `pgnimbus-test` Docker Postgres, computer-use per-char keystrokes) in both themes: `from dr` → `daily_revenue` top + preselected (impossible before — strict prefix offered only DROP), Enter inserted `analytics.daily_revenue`; `from ord` → `orders` preselected above `order_items`; `o.ta` fuzzy-narrowed member columns to `total_amount`; backspace re-expands the list; no-match (`from xq`) closes the popup. Regression-checked: FROM auto-open, JOIN FK float (`order_items`/`customers` "· FK" first), `ON ` one-keystroke `oi.order_id = o.id`, `coale`→`coalesce()`, string-literal suppression — all intact. | `(this commit)` |

## Backlog batch — 2026-07-14 (branch `claude/backlog-prioritization-b79po5`)
...
| Item | Outcome | Commit |
| --- | --- | --- |
| Privacy guarantee (README) | A Privacy section pledging zero telemetry / zero network traffic beyond the configured DB + SSH hosts; Polish backlog entry checked off. | `e1a2206` |
| Find & replace in the editor | AvaloniaEdit's `SearchPanel` installed on the SQL editor. Ctrl/Cmd+F find, Ctrl/Cmd+H replace (both through `Hotkeys`, seeded with the single-line selection; a repeat press refocuses the box via `Reactivate`), F3/Shift+F3 step, Esc closes, match wash reuses the bracket-highlight accent per theme, palette actions "Find in editor" / "Find & replace in editor". Verified both themes: 4-of-4 match count, Replace All rewrote every occurrence, palette Enter reopens with the pattern selected. | `c533a75` |
| Follow a foreign key from the grid | New Core `ForeignKeyNavigator` (unit-tested): forward hop (cell's column is an FK child → the parent row) and reverse hops (key cell → every table whose FK points here), plus `BuildFilter` rendering the browse `WHERE` (`order_id = 3`, composite keys AND-joined, SQL-literal quoting via the now-public `ResultExporter.FormatSqlLiteral`, NULL key → no hop). The grid context menu composes "Follow *col* → *parent*" / "Referencing rows ▸" per pressed cell on `ContextMenu.Opening`, reading `MainViewModel`'s background-warmed FK cache (invalidated with the other catalog caches on refresh); each hop opens the target in a new browse tab via `PreviewTableAsync`'s new initial-filter overload. Table identity comes from browse mode / the edit context, so arbitrary joins deliberately offer nothing. **Landmine:** a string `MenuItem.Header` eats `_` as the access-key marker — `customer_id` rendered as "customerid" until doubled. Verified live: orders.customer_id → customers `id = 4` (the row), orders.id → "Referencing rows ▸ sales.order_items · order_id" → 3 rows `order_id = 3`, non-key cells show neither item. | `(this commit)` |

## Backlog batch — 2026-07-21 (branch `claude/readme-backlog-priority-s1glb6`)

Top "Next up" roadmap item that's individually shippable in this codebase,
verified live under Xvfb against a staged lock-wait scene before landing.

| Item | Outcome | Commit |
| --- | --- | --- |
| Locks & blocking tree | The README "Locks & blocking tree" roadmap item: a who-blocks-whom view inside the Server Activity window, with one-click cancel/terminate of the *blocker*. The window is now two tabs — the existing flat `pg_stat_activity` grid ("Backends"), and a new "Blocking" lock tree. Core: `ActivityService.GetBlockingAsync` reads `pg_blocking_pids(pid)` (authoritative over a hand-rolled `pg_locks` self-join — it understands lock groups / parallel workers) for the blocker edges plus one ungranted `pg_locks` row per waiter (relation oid resolved to a name) for the "waiting for *mode* on *object*" label; a pure, unit-tested `Monitoring/BlockingTree.Build` (sibling of `Json/JsonTree`) shapes the flat rows into a blocker→blocked forest — roots are the ultimate lock holders. The builder is hardened against the awkward shapes a live server produces: chains (A blocks B blocks C nest in order), a waiter with several holders (appears under each), an invisible out-of-snapshot blocker like autovacuum (waiter promoted to a root), and a transient deadlock cycle (guarded so it terminates and surfaces both pids once) — 9 TUnit cases assert all of it (full suite 513 green). App: `ActivityViewModel` grows `BlockingRoots` + `SelectedBlockingNode`; the 2s auto-refresh ticks both tabs, and the tree's nodes auto-expand (a `TreeViewItem` style setting `IsExpanded=True`) so the whole wait chain shows at a glance and survives the clear-and-refill rebuild. Cancel/terminate on the Blocking tab target the selected node's pid (aim at the root holder to release everyone beneath); Terminate goes through the destructive ConfirmDialog, Cancel fires directly — same contract as the Backends tab. Verified live under Xvfb with a staged holder/waiter scene (`UPDATE … WHERE id=1` held open under a `pg_sleep`, a second `UPDATE` on the same row blocked): the Blocking tab showed the holder (pid, `postgres@demo`, "blocking 1") with the blocked `UPDATE` nested beneath in amber ("waiting for ShareLock on transactionid · 2:19"), selecting the waiter enabled Cancel/Terminate, and clearing the locks flipped the tab to the "No backend is waiting on a lock right now." empty state. | `(this commit)` |
| Elegant control vocabulary, applied app-wide | A UI polish pass, then rolled out across every secondary window + dialog (main window's minimalist command bar deliberately left as-is). New `Styles/Theme.axaml` classes: `TabControl.segmented` (a retemplated macOS-style segmented capsule for horizontal tab strips — the header sits in a subtle card capsule, each segment is a centered pill fading to the brand-blue wash + accent text when selected, with a `BrushTransition`; retemplated so it never inherits the vertical left-nav `TabItem` geometry or Fluent's underline pipe), `Button.soft` (a tactile card-toned outline pill with a hairline border lifting to the accent tint on hover/press, pinned at `PART_ContentPresenter` since Fluent repaints it), `Button.soft.danger` (outline red for secondary destructive actions) and filled `Button.danger` (mirrors `.accent` in red, for the affirmative of a destructive confirm), backed by theme-independent `AppDanger*` tokens. Applied: Server Activity + Database Overview tabs → `segmented`, their Refresh + all secondary/Cancel buttons → `soft`, the two Terminate → `soft danger`; the shared `ConfirmDialog` (always destructive — Terminate/Drop/Delete/Discard) confirm → filled `danger`; per-dialog rows normalized to the accent(primary)/soft(secondary)/danger(destructive) trio — ConnectionDialog (Delete → soft danger, Test/Save/New → soft, Connect → accent), ImportDialog, AddRowDialog, AlterTableDialog (Drop column → soft danger), PendingChangesDialog (dropped an ad-hoc `#E03131` for `soft danger`). Documented as UI design rule 6 (shared control vocabulary). Verified live under Xvfb in both the ConnectionDialog (all four button variants side by side), Database Overview (segmented strip), and a live ConfirmDialog (filled-red Terminate + soft Cancel). | `(this commit)` |

## Bug fix — 2026-08-27 (branch `claude/left-panel-search-bug-agrvnj`)

| Item | Outcome | Commit |
| --- | --- | --- |
| Sidebar filter missed tables in collapsed schemas | Reported as "search in the left panel seems to take what is on screen: it did not show a table that exists, and the palette found it". That was exactly right. `SchemaTreeViewModel.ApplyFilter` walked `Schemas` and each node's already-loaded `Children`, and the tree loads a schema's tables lazily on first expand, so a table in a never-expanded schema could not match and its schema was filtered out. The command palette searches `SchemaService.GetAllRelationsAsync`, which is why Ctrl+K found the same table. The filter now matches that same whole-catalog list: `OnFilterTextChanged` runs the synchronous pass over the loaded tree first (typing stays responsive), then awaits the relation snapshot and re-runs, with a generation counter dropping a late fetch the user has already typed past. A schema whose unloaded tables match is shown and expanded, and because the children that then arrive default to visible, the VM watches each schema's `Children` collection and re-vets them against the live filter, so only the matches end up on screen. A loaded schema is still judged by its own children (fresher than the snapshot). The snapshot is host-supplied through the new `AllRelationsRequested`, wired to a `MainViewModel.GetRelationsAsync` that the palette now shares, so both surfaces get one query per connection and one invalidation in `RefreshSchemaAsync`; a failed fetch is swallowed (the filter degrades to the loaded tree, the tree's own error strip stays for real tree errors) and retried on the next keystroke. **Tests**: 5 new cases in a new `PgNimbus.App.Tests/SchemaTreeFilterTests` (collapsed-schema match, children vetted after the triggered load, loaded schema judged by its own children, clear restores everything, failed fetch degrades) — confirmed they go red against the old behavior before landing; App suite 48 green, Core 750 green. **Verified live under Xvfb** against a seeded three-schema database with everything collapsed: `invoice_lin` revealed `billing` expanded to `invoice_lines` alone (before the fix the tree went empty), `cust` did the same for `public.customers`, `event` for `analytics.events`, and the ✕ restored the full tree. | `(this commit)` |
