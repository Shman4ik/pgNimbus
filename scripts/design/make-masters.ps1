# Renders every design master from the two committed SVG marks.
#
#   INPUT  design/logo.svg + design/logo-dark.svg   (generated by af-to-svg.py)
#
...
#          design/masters/logo/logo-{light,dark}.png             README header, 1024
...
$dark  = Join-Path $designDir 'logo-dark.svg'
foreach ($p in @($light, $dark)) { if (-not (Test-Path $p)) { throw "Missing $p - run scripts/design/af-to-svg.py first." } }
...
# window-light = for LIGHT surfaces, so the field left behind must be the dark
# ink: that is logo-dark.svg's palette. window-dark is the mirror. The name is
# the theme the icon is used *on*, not the colour it is drawn in.
foreach ($pair in @(
        @{ Src = $dark;  Dst = 'window-light-256.png' },
...
# ------------------------------------------------------- README header marks
# The full mark at 1024 on transparency. logo-light is for light backgrounds
# (so: the ink plate), logo-dark for dark ones - same naming rule as the window
# masters. icon-1024.png is the same render; these exist because the README and
# the website reference masters/logo/ and would otherwise reach across into the
# icon directory for a file that means something else.
Export-Png $light (Join-Path $logoDir 'logo-light.png') 1024
Export-Png $dark  (Join-Path $logoDir 'logo-dark.png')  1024
...
foreach ($v in @(
        ...
        @{ Src = $dark;  Fill = '#F5F7FA'; Name = 'wordmark-dark' })) {
